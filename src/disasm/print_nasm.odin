...
print_nasm_rx_op :: proc(w: io.Writer, rx: RX_Op) -> (err: io.Error) {
...
print_nasm_rm_op :: proc(w: io.Writer, rm: RM_Op) -> (err: io.Error) {
...
print_nasm_eop :: proc(w: io.Writer, addr: u64, inst: Instruction) -> (err: io.Error) {
    eop := inst.extra_op
...
print_nasm :: proc(w: io.Writer, addr: u64, inst: Instruction) -> (err: io.Error) {
    ...
    if .Direction_Bit not_in inst.flags {
        if inst.rm_op.kind != .None {
            io.write_byte(w, ' ') or_return
            print_nasm_rm_op(w, inst.rm_op) or_return
            n += 1
        }
        if inst.rx_op.kind != .None {
            ...
            io.write_byte(w, ' ') or_return
            print_nasm_rx_op(w, inst.rx_op) or_return
            n += 1
        }
    } else {
        if inst.rx_op.kind != .None {
    ...
    if inst.extra_op.kind != nil {
        ...
        io.write_byte(w, ' ') or_return
        print_nasm_eop(w, addr, inst) or_return
        n += 1
    }
    return .None
}
